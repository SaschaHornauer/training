...
\    :return: loss
    ...
    if input[0].size(0) < config['training']['dataset']['batch_size'] / 3 + 1:
        return 0.
    ...
    # outputs = torch.unbind(outputs, 1)
...
def main():
    ...
    for epoch in range(config['training']['start_epoch'], config['training']['num_epochs']):
        try:
            ...
            Utils.save_net(config['model']['save_path'], config['model']['name'] + "epoch%04d" % (epoch,), net)
